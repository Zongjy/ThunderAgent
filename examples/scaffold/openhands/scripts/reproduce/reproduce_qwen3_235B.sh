...
# Reproduce Qwen3-235B-A22B-Instruct-2507 + vLLM + ThunderAgent + OpenHands SWE-bench run.
...
MODEL_DIR="${HF_HOME}/models/qwen3-235b-a22b-instruct-2507"
...
SWEBENCH_SPLIT="test"
SWEBENCH_MAX_ITERATIONS="50"
SWEBENCH_WORKERS="128"
SWEBENCH_OUTPUT_DIR="test"
...
# Internal wiring
# =========================
VLLM_LOG="${LOG_DIR}/vllm_qwen3_${VLLM_PORT}.log"
...
run_swebench() {
  ...
  python -m evaluation.benchmarks.swe_bench.run_infer \
    ...
    --eval-output-dir "${SWEBENCH_OUTPUT_DIR}"
}

main() {
  [[ -n "${HF_HOME}" ]] || die "HF_HOME is required. Edit HF_HOME at the top of this script."
  [[ -d "./examples/scaffold/openhands" ]] || die "Please run from repository root."
